...
if (!addr->host_list)
  {
  deliver_host = deliver_host_address = cUS("");
  deliver_host_port = 0;
  }
else
  {
...
g = string_append(g, 2, cUS(" H="), h->name);

if (LOGGING(dnssec) && h->dnssec == DS_YES)
  g = string_catn(g, cUS(" DS"), 3);

g = string_append(g, 3, cUS(" ["), h->address, cUS("]"));
...
if (LOGGING(proxy) && proxy_local_address)
  {
  g = string_append(g, 3, cUS(" PRX=["), proxy_local_address, cUS("]"));
  if (LOGGING(outgoing_port))
...
if (testflag(addr, af_tcp_fastopen))
  g = string_catn(g, cUS(" TFO*"), testflag(addr, af_tcp_fastopen_data) ? 5 : 4);
...
if (LOGGING(tls_cipher) && addr->cipher)
  {
  g = string_append(g, 2, cUS(" X="), addr->cipher);
...
if (LOGGING(tls_peerdn) && addr->peerdn)
  g = string_append(g, 3, cUS(" DN=\""), string_printing(addr->peerdn), cUS("\""));
return g;
}
#endif
...
if (!addr->transport)
  ...
  if (Ustrcmp(event, "msg:fail:delivery") == 0)
    ...
    (void) event_raise(event_action, cUS("msg:fail:internal"), addr->message, NULL);
    }
  }
else
  {
...
if (msg)
  g = string_append(g, 2, host_and_ident(TRUE), cUS(" "));
else
  {
...
if (used_return_path && LOGGING(return_path_on_delivery))
  g = string_append(g, 3, cUS(" P=<"), used_return_path, cUS(">"));

if (msg)
  g = string_append(g, 2, cUS(" "), msg);

/* For a delivery from a system filter, there may not be a router */
if (addr->router)
  g = string_append(g, 2, cUS(" R="), addr->router->name);

g = string_append(g, 2, cUS(" T="), addr->transport->name);
...
if (addr->transport->info->local)
  {
  if (addr->host_list)
    g = string_append(g, 2, cUS(" H="), addr->host_list->name);
  g = d_log_interface(g);
...
/* Remote delivery */
  ...
  if (addr->host_used)
    ...
    /* DNS lookup status */
    lookup_dnssec_authenticated = addr->host_used->dnssec==DS_YES ? cUS("yes")
			      : addr->host_used->dnssec==DS_NO ? cUS("no")
			      : NULL;
#endif
    }
  ...
  if (addr->authenticator)
    {
    g = string_append(g, 2, cUS(" A="), addr->authenticator);
    if (addr->auth_id)
      {
      g = string_append(g, 2, cUS(":"), addr->auth_id);
      if (LOGGING(smtp_mailauth) && addr->auth_sndr)
        g = string_append(g, 2, cUS(":"), addr->auth_sndr);
      }
    }
...
if (  LOGGING(smtp_confirmation)
  ...
  *p++ = '\"';
  *p = 0;
  g = string_append(g, 2, cUS(" C="), big_buffer);
  }

/* Time on queue and actual time taken to deliver */

if (LOGGING(queue_time))
  g = string_append(g, 2, cUS(" QT="), string_timesince(
    LOGGING(queue_time_exclusive) ? &received_time_complete : &received_time));

if (LOGGING(deliver_time))
  g = string_append(g, 2, cUS(" DT="), string_timediff(&addr->delivery_time));
...
#ifndef DISABLE_EVENT
if (!msg) msg_event_raise(cUS("msg:delivery"), addr);
#endif
...
if (driver_name)
  ...
  g = string_append(g, 2, cUS(" "), driver_kind);

g = string_fmt_append(g, " defer (%d)", addr->basic_errno);

if (addr->basic_errno > 0)
  g = string_append(g, 2, cUS(": "), US(strerror(addr->basic_errno)));
...
if (addr->message)
  g = string_append(g, 2, cUS(": "), addr->message);
...
if (  !addr->prop.ignore_error
  ...
  msg_event_raise(cUS("msg:fail:delivery"), addr);
#endif
...
if (LOGGING(sender_on_delivery))
  g = string_append(g, 3, cUS(" F=<"), sender_address, cUS(">"));
...
if (driver_type == EXIM_DTYPE_TRANSPORT)
  ...
  if (addr->transport)
    ...
    driver_kind = cUS(" transport");
    f.disable_logging = addr->transport->disable_logging;
    }
  else driver_kind = cUS("transporting");
  }
  ...
  if (addr->router)
    ...
    driver_kind = cUS(" router");
    f.disable_logging = addr->router->disable_logging;
    }
  else driver_kind = cUS("routing");
  }
    ...
    if (sender_address[0] != 0 || addr->prop.errors_address)
      if (tb->return_output)
        ...
        if (addr->basic_errno == 0 && !addr->message)
          addr->message = cUS("return message generated");
        return_output = TRUE;
        }
      else
...
explicitly requested by a router or transport. */
  ...
  if (  !addr->prop.ignore_error
    ...
    frozen_info = addr->special_action == SPECIAL_FREEZE
      ...
      ? cUS(" (message created with -f <>)")
      : cUS(" (delivery error message)");
    f.deliver_freeze = TRUE;
    deliver_frozen_at = time(NULL);
    update_spool = TRUE;
...
if (tp->gid_set)
  ...
  *gidp = tp->gid;
  ...
  {
  if (!route_find_expanded_group(tp->expand_gid, tp->name, cUS("transport"), gidp,
    &(addr->message)))
    {
...
it does not provide a passwd value from which a gid can be taken. */
  ...
  {
  struct passwd *pw;
  if (!route_find_expanded_user(tp->expand_uid, tp->name, cUS("transport"), &pw,
       uidp, &(addr->message)))
    {
...
if (tp->return_path)
  ...
  if (new_return_path)
    ...
    common_error(TRUE, addr, ERRNO_EXPANDFAIL,
      cUS("Failed to expand return path \"%s\" in %s transport: %s"),
...
else working_directory = deliver_home ? deliver_home : cUS("/");
...
if (pipe(pfd) != 0)
  {
  common_error(TRUE, addr, ERRNO_PIPEFAIL, cUS("Creation of pipe failed: %s"),
    strerror(errno));
  return;
  }
...
if ((pid = exim_fork(cUS("delivery-local"))) == 0)
  {
  BOOL replicate = TRUE;
  ...
  /* If successful, call the transport */
    ...
    if (tp->filter_command)
      ...
      ok = transport_set_up_command(&transport_filter_argv,
        tp->filter_command,
        TRUE, PANIC, addr, FALSE, cUS("transport filter"), NULL);
...
if (addr->special_action == SPECIAL_WARN && addr->transport->warn_message)
  ...
  else if ((pid = child_open_exim(&fd, cUS("tpt-warning-message"))) > 0)
    {
    FILE *f = fdopen(fd, "wb");
    if (errors_reply_to && !contains_header(cUS("Reply-To"), warn_message))
...
if (max_parallel > 0)
  ...
  if (!enq_start(serialize_key, max_parallel))
    ...
    do
      {
      next = addr->next;
      addr->message = cUS("concurrency limit reached for transport");
...
while (addr_local)
  ...
  if (!(tp = addr->transport))
    ...
    addr->message = addr->router
      ...
      : cUS("No transport set by system filter");
  ...
  if (tp->batch_max > 1 && addr_local)
    {
    int batch_count = 1;
    BOOL uses_dom = readconf_depends((driver_instance *)tp, cUS("domain"));
    BOOL uses_lp = (  testflag(addr, af_pfr)
		   && (testflag(addr, af_file) || addr->local_part[0] == '|')
		   )
		|| readconf_depends((driver_instance *)tp, cUS("local_part"));
  ...
  while (addr2)
    ...
    *retry_key = 'T';
    ...
    because post processing puts it on another chain. */
      ...
      this->message = cUS("Retry time not yet reached");
  ...
  if (  tp->shadow
     && (  !tp->shadow_condition
        || expand_check_condition(tp->shadow_condition, tp->name, cUS("transport"))
     )  )
    {
...
cuschar *listptr = remote_sort_domains;
uschar *pattern;
uschar patbuf[256];
...
while (  *aptr
  ...
  while (*aptr)
    ...
    if (match_isinlist(deliver_domain, (cuschar **)&pattern, UCHAR_MAX+1,
    ...
    while (  *next
	  && (deliver_domain = (*next)->domain,  /* Set $domain */
            match_isinlist(deliver_domain, (cuschar **)&pattern, UCHAR_MAX+1,
              &domainlist_anchor, NULL, MCL_DOMAIN, TRUE, NULL)) != OK
	  )
      next = &(*next)->next;
...
for (int delivery_count = 0; addr_remote; delivery_count++)
  ...
  if (!(tp = addr->transport))
    {
    f.disable_logging = FALSE;  /* Jic */
    panicmsg = cUS("No transport set by router");
    goto panic_continue;
    }
...
*/
  ...
  if (exp_bool(addr, cUS("transport"), tp->name, D_transport,
		cUS("multi_domain"), tp->multi_domain, tp->expand_multi_domain,
		&multi_domain) != OK)
    {
    deliver_set_expansions(NULL);
    ...
    if (  (multi_domain || Ustrcmp(next->domain, addr->domain) == 0)
	  ...
	  || (  (
		(void)(!tp->expand_multi_domain || ((void)deliver_set_expansions(next), 1)),
	        exp_bool(addr,
		    cUS("transport"), next->transport->name, D_transport,
		    cUS("multi_domain"), next->transport->multi_domain,
...
*/
  ...
  if (poffset >= remote_max_parallel)
    ...
    (void)close(pfd[pipe_write]);
    (void)close(pfd[pipe_read]);
    panicmsg = cUS("Unexpectedly no free subprocess slot");
    goto enq_continue;
    }
  ...
  if ((pid = exim_fork(cUS("transport"))) == 0)
    {
    ...
    {
    uschar * fname = spool_fname(cUS("input"), message_subdir, message_id, cUS("-D"));
...
/*um, are they really per-addr?  Other per-conn stuff is not (auth, tls).  But host_used is! */
  ...
  if (cutthrough.cctx.sock >= 0 && cutthrough.callout_hold_only)
    ...
    release_cutthrough_connection(cUS("passed to transport proc"));
    }
...
if (percent_hack_domains)
  ...
  while (  (rc = match_isinlist(deliver_domain, (cuschar **)&percent_hack_domains, 0,
...
if (addr->parent && testflag(addr, af_hide_child))
  {
  printed = cUS("an undisclosed address");
  yield = FALSE;
  }
...
static void
print_address_error(address_item * addr, FILE * f, cuschar * t)
{
int count = Ustrlen(t);
...
static FILE *
expand_open(cuschar * filename,
  cuschar * varname, cuschar * reason)
{
cuschar * s = expand_cstring(filename);
FILE * fp = NULL;
...
report_time_since(&timestamp_startup, cUS("delivery start"));	/* testcase 0022, 2100 */
#endif
...
give up; if the message has been around for sufficiently long, remove it. */
  ...
  if ((rc = spool_read_header(spoolname, TRUE, TRUE)) != spool_read_OK)
    ...
    if (errno == ERRNO_SPOOLFORMAT)
      {
      struct stat statbuf;
      if (Ustat(spool_fname(cUS("input"), message_subdir, spoolname, cUS("")),
		&statbuf) == 0)
    ...
    if (now - received_time.tv_sec > keep_malformed)
      {
      Uunlink(spool_fname(cUS("msglog"), message_subdir, id, cUS("")));
      Uunlink(spool_fname(cUS("input"), message_subdir, id, cUS("-D")));
      Uunlink(spool_fname(cUS("input"), message_subdir, id, cUS("-H")));
      Uunlink(spool_fname(cUS("input"), message_subdir, id, cUS("-J")));
...
Otherwise it might be needed again. */
  ...
  {
  uschar * fname = spool_fname(cUS("input"), message_subdir, id, cUS("-J"));
  FILE * jread;
...
if (f.deliver_freeze)
  ...
  if (  move_frozen_messages
     && spool_move_message(id, message_subdir, cUS(""), cUS("F"))
     )
    return continue_closedown();   /* yields DELIVER_NOT_ATTEMPTED */
#endif
...
logged, and the delivery attempt fails. */
  ...
  rc = rda_interpret(
    ...
    cUS("system filter"));     /* For error messages */
  ...
  work properly. */
    ...
    frozen_info = string_sprintf(" by the system filter%s%s",
      filter_message ? cUS(": ") : cUS(""),
      filter_message ? filter_message : cUS(""));
    }
  ...
  if (addr_new)
    ...
    address_item *parent = deliver_make_addr(cUS("system-filter"), FALSE);

    parent->domain = string_copylc(qualify_domain_recipient);
    parent->local_part = cUS("system-filter");
...
if (process_recipients != RECIP_IGNORE)
  for (i = 0; i < recipients_count; i++)
	...
	new->dsn_orcpt ? new->dsn_orcpt : cUS(""), new->dsn_flags);
	  ...
	  new->message =
	    filter_message ? filter_message : cUS("delivery cancelled");
	  ...
	  new->message  = cUS("delivery cancelled; message timed out");
	  goto RECIP_QUEUE_FAILED;   /* below */
        ...
        case RECIP_FAIL:
	  new->message  = cUS("delivery cancelled by administrator");
	  /* Fall through */
	  ...
	  new->message = cUS("Too many \"Received\" headers - suspected mail loop");
	...
	if (!parse_extract_address(addr, &errmsg, &start, &end, &dom, TRUE))
	  ...
	  (void) event_raise(event_action, cUS("msg:fail:internal"), new->message, NULL);
...
DEBUG(D_deliver)
  ...
  for (address_item * p = addr_new; p; p = p->next)
    debug_printf("  %s %s\n", p->address,
      p->onetime_parent ? p->onetime_parent : cUS(""));
  }
...
while (addr_new)           /* Loop until all addresses dealt with */
  ...
  while (addr_new)
    ...
    if (testflag(addr, af_pfr))
      ...
      if (testflag(addr, af_bad_reply))
        ...
        addr->message =
          cUS("filter autoreply generated syntactically invalid recipient");
        addr->prop.ignore_error = TRUE;
      ...
      if (testflag(addr, af_file))
        ...
        if (!testflag(addr, af_allow_file))
          ...
          addr->message = cUS("delivery to file forbidden");
          (void)post_process_one(addr, FAIL, LOG_MAIN, EXIM_DTYPE_ROUTER, 0);
          continue;   /* with the next new address */
          }
        }
        ...
        if (!testflag(addr, af_allow_pipe))
          ...
          addr->message = cUS("delivery to pipe forbidden");
          (void)post_process_one(addr, FAIL, LOG_MAIN, EXIM_DTYPE_ROUTER, 0);
          continue;   /* with the next new address */
          }
        }
        ...
        addr->message = cUS("autoreply forbidden");
        (void)post_process_one(addr, FAIL, LOG_MAIN, EXIM_DTYPE_ROUTER, 0);
        continue;     /* with the next new address */
        }
	...
	*t = *save_t;
    ...
    if ((rc = deliver_split_address(addr)) == DEFER)
      {
      addr->message = cUS("cannot check percent_hack_domains");
    ...
    if (  !forced && hold_domains
       && (rc = match_isinlist(addr->domain, (cuschar **)&hold_domains, 0,
      ...
      if (rc == DEFER)
        {
        addr->message = cUS("hold_domains lookup deferred");
        addr->basic_errno = ERRNO_LISTDEFER;
        }
      else
        {
        addr->message = cUS("domain is held");
    ...
    if (continue_hostname && domain_retry_record)
      {
      addr->message = cUS("reusing SMTP connection skips previous routing defer");
    ...
    deferring messages. */
      ...
      {
      addr->message = cUS("retry time not reached");
      addr->basic_errno = ERRNO_RRETRY;
      (void)post_process_one(addr, DEFER, LOG_MAIN, EXIM_DTYPE_ROUTER, 0);
    ...
    while (addr_route)
      ...
      if ((rc = match_isinlist(addr->domain, CUSS(&queue_domains), 0,
        ...
        if (rc == DEFER)
          {
          addr->basic_errno = ERRNO_LISTDEFER;
          addr->message = cUS("queue_domains lookup deferred");
          (void)post_process_one(addr, DEFER, LOG_MAIN, EXIM_DTYPE_ROUTER, 0);
          }
        else
          {
        ...
        addr->message = cUS("domain is in queue_domains");
        (void)post_process_one(addr, DEFER, LOG_MAIN, EXIM_DTYPE_ROUTER, 0);
        }
      }
  ...
  while (addr_route)
    ...
    int rc;
    address_item *addr = addr_route;
    cuschar *old_domain = addr->domain;
...
if (f.queue_run_local)
  while (addr_remote)
    ...
    addr->message = cUS("remote deliveries suppressed");
...
cancel_cutthrough_connection(TRUE, cUS("deliveries are done"));

/* Root privilege is no longer needed */

exim_setugid(exim_uid, exim_gid, FALSE, cUS("post-delivery tidying"));
...
for (address_item * a = addr_succeed; a; a = a->next)
  ...
  DEBUG(D_deliver) debug_printf("DSN: processing router : %s\n"
      ...
      a->router ? a->router->name : cUS("(unknown)"),
      a->address,
      sender_address,
      a->dsn_orcpt ? a->dsn_orcpt : cUS("NULL"),
      a->dsn_flags,
      dsn_envid ? dsn_envid : cUS("NULL"), dsn_ret,
      a->address,
      a->dsn_aware
      );
...
if (addr_senddsn)
  ...
  pid = child_open_exim(&fd, cUS("DSN"));
  ...
  if (pid < 0)  /* Creation of child failed */
    ...
    for (address_item * a = addr_senddsn; a; a = a->next)
      ...
      print_dsn_addr_action(f, a, cUS("delivered"), cUS("2.0.0"));
...
while (addr_failed)
  ...
  if (  addr_failed->prop.ignore_error
    ...
    log_write(0, LOG_MAIN, "%s%s%s%s: error ignored%s",
      addr->address,
      !addr->parent ? cUS("") : cUS(" <"),
      !addr->parent ? cUS("") : addr->parent->address,
      !addr->parent ? cUS("") : cUS(">"),
      addr->prop.ignore_error
      ? cUS("") : cUS(": RFC 3461 DSN, failure notify not requested"));
  ...
  error message. */
    ...
    if ((pid = child_open_exim(&fd, cUS("bounce-message"))) < 0)
      log_write(0, LOG_MAIN|LOG_PANIC_DIE, "Process %d (parent %d) failed to "
	...
	emf = expand_open(bounce_message_file,
		cUS("bounce_message_file"), cUS("error"));
	...
	fprintf(fp, "Bcc: %s\n", bcc);
      ...
      if ((emf_text = next_emf(emf, cUS("header"))))
	fprintf(fp, "%s\n", emf_text);
      else
	...
	bound);
      ...
      if ((emf_text = next_emf(emf, cUS("intro"))))
...
"address(es) failed:\n", sender_address);
      ...
      for (addr = msgchain; addr; addr = *paddr)
        {
        if (print_address_information(addr, fp, cUS("  "), cUS("\n    "), cUS("")))
          print_address_error(addr, fp, cUS(""));
      ...
      emf_text = next_emf(emf, cUS("generated text"));
	  ...
	  fputc('\n', fp);
          while(addr)                   /* Insurance */
            {
            print_address_information(addr, fp, cUS("------ "),  cUS("\n       "),
              cUS(" ------\n"));
	...
	print_dsn_addr_action(fp, addr, cUS("failed"), cUS("5.0.0"));
	...
	fputc('\n', fp);
      ...
      emf_text = next_emf(emf, cUS("copy"));
      ...
      */
      ...
      dsnlimitmsg = cUS("X-Exim-DSN-Information: Due to administrative limits only headers are returned");
      dsnnotifyhdr = NULL;
...
if (!addr_defer)
  ...
  if (message_logs)
    {
    fname = spool_fname(cUS("msglog"), message_subdir, id, cUS(""));
    if (preserve_message_logs)
      {
      int rc;
      uschar * moname = spool_fname(cUS("msglog.OLD"), cUS(""), id, cUS(""));

      if ((rc = Urename(fname, moname)) < 0)
        {
        (void)directory_make(spool_directory,
			      spool_sname(cUS("msglog.OLD"), cUS("")),
			      MSGLOG_DIRECTORY_MODE, TRUE);
  ...
  if (Uunlink(fname) < 0)
    log_write(0, LOG_MAIN|LOG_PANIC_DIE, "failed to unlink %s: %s",
      fname, strerror(errno));
  fname = spool_fname(cUS("input"), message_subdir, id, cUS("-H"));
  if (Uunlink(fname) < 0)
    log_write(0, LOG_MAIN|LOG_PANIC_DIE, "failed to unlink %s: %s",
      fname, strerror(errno));
...
#ifndef DISABLE_EVENT
  (void) event_raise(event_action, cUS("msg:complete"), NULL, NULL);
#endif
  }
...
*/
  ...
  for (address_item * addr = addr_defer; addr; addr = addr->next)
    ...
    if (deliver_domain)
      {
      cuschar *d = testflag(addr, af_pfr)
  ...
  if (  !f.queue_2stage
    ...
    if (warning_count < count)
      ...
      header_line *h;
      int fd;
      pid_t pid = child_open_exim(&fd, cUS("delay-warning-message"));
	...
	transport_ctx tctx = {{0}};
	  ...
	  wmf = expand_open(warn_message_file,
		  cUS("warn_message_file"), cUS("warning"));
	...
	moan_write_references(f, NULL);
	  ...
	  bound);
        ...
        if ((wmf_text = next_emf(wmf, cUS("header"))))
          fprintf(f, "%s\n", wmf_text);
        else
	  ...
	  bound);
        ...
        if ((wmf_text = next_emf(wmf, cUS("intro"))))
...
"has not yet been delivered to one or more of its recipients after more than \n",
	    ...
	    warnmsg_delay, primary_hostname, message_id);
          ...
          for (h = header_list; h; h = h->next)
            if (strncmpic(h->text, cUS("Subject:"), 8) == 0)
              fprintf(f, "The subject of the message is: %s", h->text + 9);
            else if (strncmpic(h->text, cUS("Date:"), 5) == 0)
	...
	for (address_item * addr = addr_defer; addr; addr = addr->next)
          {
          if (print_address_information(addr, f, cUS("  "), cUS("\n    "), cUS("")))
            print_address_error(addr, f, cUS("Delay reason: "));
          fputc('\n', f);
          }
        fputc('\n', f);
        ...
        if (wmf)
          {
          if ((wmf_text = next_emf(wmf, cUS("final"))))
	    fprintf(f, "%s", CS(wmf_text));
          (void)fclose(wmf);
          }
        else
...
"and when that happens, the message will be returned to you.\n");
	...
	for (address_item * addr = addr_defer; addr; addr = addr->next)
	  ...
	  print_dsn_addr_action(f, addr, cUS("delayed"), cUS("4.0.0"));
  ...
  if (f.deliver_freeze)
    ...
    if (freeze_tell && freeze_tell[0] != 0 && !f.local_error_message)
      ...
      moan_tell_someone(freeze_tell, addr_defer, cUS("Message frozen"),
...
if (remove_journal)
  ...
  if (f.deliver_freeze && move_frozen_messages)
    (void)spool_move_message(id, message_subdir, cUS(""), cUS("F"));
#endif
  }
...
#ifdef MEASURE_TIMING
report_time_since(&timestamp_startup, cUS("delivery end")); /* testcase 0005 */
#endif
...
if (cutthrough.cctx.sock >= 0 && cutthrough.callout_hold_only)
  ...
  {
  cancel_cutthrough_connection(TRUE, cUS("non-continued delivery"));
  (void) child_exec_exim(exec_type, FALSE, NULL, FALSE, 2, cUS("-Mc"), message_id);
  }
