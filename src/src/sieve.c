...
cuschar *exim_sieve_extension_list[] = {
...
for (int pass = 0; pass <= 1; pass++)
  ...
  for (cuschar * start = src->character, * end = start + src->length;
       start < end; ++start)
    {
    ch=*start;
...
static int
parse_mailto_uri(struct Sieve *filter, cuschar *uri,
...
if (Ustrncmp(uri,"mailto:",7))
  {
  filter->errmsg=cUS("Unknown URI scheme");
  return 0;
  }
...
if (*uri && *uri!='?')
  for (;;)
    ...
    if (uri>start)
      ...
      if (uri_decode(&to)==-1)
        {
        filter->errmsg=cUS("Invalid URI encoding");
        return -1;
        }
      ...
      *recipient = new;
      ...
      {
      filter->errmsg = cUS("Missing addr-spec in URI");
      return -1;
      }
    if (*uri=='%') uri+=3;
    else break;
...
if (*uri=='?')
  ...
  for (;;)
    ...
    if (*uri=='=')
      ...
      {
      filter->errmsg=cUS("Missing equal after hname");
      return -1;
      }
    /* match hvalue */
    for (start=uri; *uri && (isalnum(*uri) || strchr("$-_.+!*'(),%",*uri)); ++uri);
    ...
    if (hname.length==2 && strcmpic(hname.character, cUS("to"))==0)
      {
      new=store_get(sizeof(string_item), GET_UNTAINTED);
    ...
    else if (hname.length==4 && strcmpic(hname.character, cUS("body"))==0)
      *body=hvalue;
    else if (hname.length==7 && strcmpic(hname.character, cUS("subject"))==0)
      *subject=hvalue;
    else
      {
...
if (*uri)
  {
  filter->errmsg=cUS("Syntactically invalid URI");
  return -1;
  }
return 1;
}
...
while (t>s && (*(t-1)==' ' || *(t-1)=='\t')) --t;
*t='\0';
value->character=rfc2047_decode(s,check_rfc2047_length,cUS("utf-8"),'\0',&value->length,&errmsg);
}
...
while (src<end)
  ...
  if (
    ...
    src=brace+1;
           ...
           strncmpic(src,cUS("${unicode:"),10)==0
...
static int parse_identifier(struct Sieve *filter, cuschar *id)
{
...
while (*filter->pc)
  ...
  if (parse_identifier(filter,cUS("if")))
    ...
    */
    ...
    if (exec)
      {
      add_addr(generated,cUS("inbox"),1,0,0,0);
      filter->keep = 0;
      }
    }
    ...
    */
    ...
    if (exec)
	  ...
	  gstring * key = NULL;
          ...
          key = string_catn(key, reason_is_mime?cUS("1"):cUS("0"), 1);
	  ...
	  md5_end(&base, handle.character, handle.length, digest);
        ...
        if (filter_test == FTEST_NONE)
	      ...
	      subject.character = string_from_gstring(g);
              ...
              {
              subject.character=cUS("Automated reply");
	  ...
	  /* deconst cast safe as we pass in a non-const item */
          addr->reply->subject = US(parse_quote_2047(subject.character, subject.length, cUS("utf-8"), TRUE));
          ...
          if (reason_is_mime)
            ...
            addr->reply->headers = cUS("MIME-Version: 1.0\n")
...
int
sieve_interpret(cuschar * filter, int options,
  cuschar * vacation_directory, cuschar * enotify_mailto_owner,
  cuschar * useraddress, cuschar * subaddress,
...
if (parse_start(&sieve, 1, generated) == 1)
  ...
  if (sieve.keep)
    {
    add_addr(generated, cUS("inbox"), 1, 0, 0, 0);
    msg = cUS("Implicit keep");
    r = FF_DELIVERED;
    }
  else
    {
    msg = cUS("No implicit keep");
    r = FF_DELIVERED;
    }
else
  {
