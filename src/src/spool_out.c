...
static void
spool_var_write(FILE * fp, cuschar * name, cuschar * val)
{
putc('-', fp);
if (is_tainted(val))
  {
...
tname = spool_fname(cUS("input"), message_subdir, cUS("hdr."), message_id);

if ((fd = spool_open_temp(tname)) < 0)
  return spool_write_error(where, errmsg, cUS("open"), NULL, NULL);
fp = fdopen(fd, "wb");
...
if (sender_helo_name) spool_var_write(fp, cUS("helo_name"), sender_helo_name);
...
if (sender_host_address)
  ...
  if (sender_host_name)
    spool_var_write(fp, cUS("host_name"), sender_host_name);
  }
if (sender_host_authenticated)
  spool_var_write(fp, cUS("host_auth"), sender_host_authenticated);
if (sender_host_auth_pubname)
  spool_var_write(fp, cUS("host_auth_pubname"), sender_host_auth_pubname);
...
if (smtp_active_hostname != primary_hostname)
  spool_var_write(fp, cUS("active_hostname"), smtp_active_hostname);
...
if (sender_ident)
  spool_var_write(fp, cUS("ident"), sender_ident);

/* Ditto for the received protocol */

if (received_protocol)
  spool_var_write(fp, cUS("received_protocol"), received_protocol);
...
if (authenticated_id)
  spool_var_write(fp, cUS("auth_id"), authenticated_id);
if (authenticated_sender)
  spool_var_write(fp, cUS("auth_sender"), zap_newlines(authenticated_sender));
...
if (local_scan_data) spool_var_write(fp, cUS("local_scan"), local_scan_data);
#endif
#ifdef WITH_CONTENT_SCAN
if (spam_bar)       spool_var_write(fp, cUS("spam_bar"),       spam_bar);
if (spam_score)     spool_var_write(fp, cUS("spam_score"),     spam_score);
if (spam_score_int) spool_var_write(fp, cUS("spam_score_int"), spam_score_int);
#endif
if (f.deliver_manual_thaw) fprintf(fp, "-manual_thaw\n");
if (f.sender_set_untrusted) fprintf(fp, "-sender_set_untrusted\n");

#ifdef EXPERIMENTAL_BRIGHTMAIL
if (bmi_verdicts) spool_var_write(fp, cUS("bmi_verdicts"), bmi_verdicts);
#endif

#ifndef DISABLE_TLS
if (tls_in.certificate_verified) fprintf(fp, "-tls_certificate_verified\n");
if (tls_in.cipher) spool_var_write(fp, cUS("tls_cipher"), tls_in.cipher);
if (tls_in.peercert)
  {
  if (tls_export_cert(big_buffer, big_buffer_size, tls_in.peercert))
    fprintf(fp, "--tls_peercert %s\n", CS(big_buffer));
  }
if (tls_in.peerdn)       spool_var_write(fp, cUS("tls_peerdn"), string_printing(tls_in.peerdn));
if (tls_in.sni)		 spool_var_write(fp, cUS("tls_sni"),    string_printing(tls_in.sni));
if (tls_in.ourcert)
  {
...
# endif
if (tls_in.ver) spool_var_write(fp, cUS("tls_ver"), tls_in.ver);
#endif
...
for (int i = 0; i < recipients_count; i++)
  ...
  cuschar *address = zap_newlines(r->address);
  ...
  if (r->pno < 0 && !r->errors_to && r->dsn_flags == 0)
    ...
    {
    cuschar *errors_to = r->errors_to ? zap_newlines(r->errors_to) : cUS("");
    /* for DSN SUPPORT extend exim 4 spool in a compatible way by
    adding new values upfront and add flag 0x02 */
    cuschar *orcpt = r->orcpt ? zap_newlines(r->orcpt) : cUS("");
...
fflush(fp);
if (fstat(fd, &statbuf))
  return spool_write_error(where, errmsg, cUS("fstat"), tname, fp);
...
if (fflush(fp) != 0 || ferror(fp))
  return spool_write_error(where, errmsg, cUS("write"), tname, fp);
...
if (EXIMfsync(fileno(fp)) < 0)
  return spool_write_error(where, errmsg, cUS("sync"), tname, fp);

/* Get the size of the file, and close it. */

if (fstat(fd, &statbuf) != 0)
  return spool_write_error(where, errmsg, cUS("fstat"), tname, NULL);
if (fclose(fp) != 0)
  return spool_write_error(where, errmsg, cUS("close"), tname, NULL);
...
if (Urename(tname, fname) < 0)
  return spool_write_error(where, errmsg, cUS("rename"), tname, NULL);
...
tname = spool_fname(cUS("input"), message_subdir, cUS("."), cUS(""));
...
if ((fd = Uopen(tname, O_RDONLY|O_DIRECTORY, 0)) < 0)
  return spool_write_error(where, errmsg, cUS("directory open"), fname, NULL);

if (EXIMfsync(fd) < 0 && errno != EINVAL)
  return spool_write_error(where, errmsg, cUS("directory sync"), fname, NULL);

if (close(fd) < 0)
  return spool_write_error(where, errmsg, cUS("directory close"), fname, NULL);
...
if (!make_link(cUS("msglog"), dest_qname, subdir, id, cUS(""), from, to, TRUE) ||
    !make_link(cUS("input"),  dest_qname, subdir, id, cUS("-D"), from, to, FALSE) ||
    !make_link(cUS("input"),  dest_qname, subdir, id, cUS("-H"), from, to, FALSE))
  return FALSE;

if (!break_link(cUS("input"),  subdir, id, cUS("-H"), from, FALSE) ||
    !break_link(cUS("input"),  subdir, id, cUS("-D"), from, FALSE) ||
    !break_link(cUS("msglog"), subdir, id, cUS(""), from, TRUE))
  return FALSE;

log_write(0, LOG_MAIN, "moved from %s%s%s%sinput, %smsglog to %s%s%s%sinput, %smsglog",
   *queue_name?"(":"", *queue_name?queue_name:cUS(""), *queue_name?") ":"",
   from, from,
   *dest_qname?"(":"", *dest_qname?dest_qname:cUS(""), *dest_qname?") ":"",
   to, to);
