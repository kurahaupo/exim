...
if (pw == NULL)
  {
  *errptr = cUS("pwcheck: malformed input - missing colon");
  return ERROR;
  }
...
int
auth_call_saslauthd(cuschar *username, cuschar *password,
  cuschar *service, cuschar *realm, uschar **errptr)
{
uschar *reply = NULL;

if (service == NULL) service = cUS("");
if (realm == NULL) realm = cUS("");
