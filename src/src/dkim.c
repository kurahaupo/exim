...
params_dkim(void)
{
builtin_macro_create_var(cUS("_DKIM_SIGN_HEADERS"), US(PDKIM_DEFAULT_SIGN_HEADERS));
builtin_macro_create_var(cUS("_DKIM_OVERSIGN_HEADERS"), US(PDKIM_OVERSIGN_HEADERS));
}
# else	/*!MACRO_PREDEF*/
...
static cuschar * dkim_collect_error = NULL;
...
uschar *
dkim_exim_query_dns_txt(cuschar * name)
{
...
if (  !dkim_verify_overall
   && dkim_verify_status
      ? Ustrcmp(dkim_verify_status, cUS("pass")) == 0
...
if (  dkim_verify_status
  ...
  if (Ustrcmp(dkim_verify_status, cUS("fail")) == 0)
    sig->verify_status = PDKIM_VERIFY_POLICY | PDKIM_VERIFY_FAIL;
  else if (Ustrcmp(dkim_verify_status, cUS("invalid")) == 0)
    sig->verify_status = PDKIM_VERIFY_POLICY | PDKIM_VERIFY_INVALID;
  else if (Ustrcmp(dkim_verify_status, cUS("none")) == 0)
    sig->verify_status = PDKIM_VERIFY_POLICY | PDKIM_VERIFY_NONE;
  else if (Ustrcmp(dkim_verify_status, cUS("pass")) == 0)
...
logmsg = string_catn(NULL, cUS("DKIM: "), 6);
if (!(s = sig->domain)) s = cUS("<UNSET>");
logmsg = string_append(logmsg, 2, "d=", s);
if (!(s = sig->selector)) s = cUS("<UNSET>");
...
if (sig->verify_status & PDKIM_VERIFY_POLICY)
  logmsg = string_append(logmsg, 5,
	    cUS(" ["), dkim_verify_status, cUS(" - "), dkim_verify_reason, cUS("]"));
else
  switch (sig->verify_status)
    {
    case PDKIM_VERIFY_NONE:
      logmsg = string_cat(logmsg, cUS(" [not verified]"));
      break;

    case PDKIM_VERIFY_INVALID:
      logmsg = string_cat(logmsg, cUS(" [invalid - "));
      switch (sig->verify_ext_status)
	{
	case PDKIM_VERIFY_INVALID_PUBKEY_UNAVAILABLE:
	  logmsg = string_cat(logmsg,
			cUS("public key record (currently?) unavailable]"));
	  break;

	case PDKIM_VERIFY_INVALID_BUFFER_SIZE:
	  logmsg = string_cat(logmsg, cUS("overlong public key record]"));
	  break;

	case PDKIM_VERIFY_INVALID_PUBKEY_DNSRECORD:
	case PDKIM_VERIFY_INVALID_PUBKEY_IMPORT:
	  logmsg = string_cat(logmsg, cUS("syntax error in public key record]"));
	  break;

	case PDKIM_VERIFY_INVALID_SIGNATURE_ERROR:
	  logmsg = string_cat(logmsg, cUS("signature tag missing or invalid]"));
	  break;

	case PDKIM_VERIFY_INVALID_DKIM_VERSION:
	  logmsg = string_cat(logmsg, cUS("unsupported DKIM version]"));
	  break;

	default:
	  logmsg = string_cat(logmsg, cUS("unspecified problem]"));
	}
      break;

    case PDKIM_VERIFY_FAIL:
      logmsg = string_cat(logmsg, cUS(" [verification failed - "));
      switch (sig->verify_ext_status)
	{
	case PDKIM_VERIFY_FAIL_BODY:
	  logmsg = string_cat(logmsg,
	       cUS("body hash mismatch (body probably modified in transit)]"));
	  break;

	case PDKIM_VERIFY_FAIL_MESSAGE:
	  logmsg = string_cat(logmsg,
		cUS("signature did not verify ")
		"(headers probably modified in transit)]");
	  break;

	case PDKIM_VERIFY_INVALID_PUBKEY_KEYSIZE:
	  logmsg = string_cat(logmsg,
		cUS("signature invalid (key too short)]"));
	  break;

	default:
	  logmsg = string_cat(logmsg, cUS("unspecified reason]"));
	}
      break;

    case PDKIM_VERIFY_PASS:
      logmsg = string_cat(logmsg, cUS(" [verification succeeded]"));
      break;
    }
...
dkim_verify_status = cUS("none");
dkim_verify_reason = cUS("");
dkim_cur_signer = id;
...
switch (what)
  {
  case DKIM_ALGO:		return cUS("");
  case DKIM_BODYLENGTH:		return cUS("9999999999999");
  case DKIM_CANON_BODY:		return cUS("");
  case DKIM_CANON_HEADERS:	return cUS("");
  case DKIM_COPIEDHEADERS:	return cUS("");
  case DKIM_CREATED:		return cUS("0");
  case DKIM_EXPIRES:		return cUS("9999999999999");
  case DKIM_HEADERNAMES:	return cUS("");
  case DKIM_IDENTITY:		return cUS("");
  case DKIM_KEY_GRANULARITY:	return cUS("*");
  case DKIM_KEY_SRVTYPE:	return cUS("*");
  case DKIM_KEY_NOTES:		return cUS("");
  case DKIM_KEY_TESTING:	return cUS("0");
  case DKIM_NOSUBDOMAINS:	return cUS("0");
  case DKIM_VERIFY_STATUS:	return cUS("none");
  case DKIM_VERIFY_REASON:	return cUS("");
  default:			return cUS("");
  }
}
...
switch (what)
  ...
  case DKIM_CANON_BODY:
    switch (dkim_cur_sig->canon_body)
      {
      case PDKIM_CANON_RELAXED:	return cUS("relaxed");
      case PDKIM_CANON_SIMPLE:
      default:			return cUS("simple");
      }
  ...
  case DKIM_VERIFY_STATUS:
    switch (dkim_cur_sig->verify_status)
      {
      case PDKIM_VERIFY_INVALID:	return cUS("invalid");
      case PDKIM_VERIFY_FAIL:		return cUS("fail");
      case PDKIM_VERIFY_PASS:		return cUS("pass");
      case PDKIM_VERIFY_NONE:
      default:				return cUS("none");
      }
  ...
  case DKIM_VERIFY_REASON:
    switch (dkim_cur_sig->verify_ext_status)
      {
      case PDKIM_VERIFY_INVALID_PUBKEY_UNAVAILABLE:
						return cUS("pubkey_unavailable");
      case PDKIM_VERIFY_INVALID_PUBKEY_DNSRECORD:return cUS("pubkey_dns_syntax");
      case PDKIM_VERIFY_INVALID_PUBKEY_IMPORT:	return cUS("pubkey_der_syntax");
      case PDKIM_VERIFY_INVALID_PUBKEY_KEYSIZE:	return cUS("pubkey_too_short");
      case PDKIM_VERIFY_FAIL_BODY:		return cUS("bodyhash_mismatch");
      case PDKIM_VERIFY_FAIL_MESSAGE:		return cUS("signature_incorrect");
      }
...
dkim_exim_sign(int fd, off_t off, uschar * prefix,
  struct ob_dkim * dkim, cuschar ** errstr)
{
cuschar * dkim_domain = NULL;
int sep = 0;
...
if ((s = dkim->dkim_domain) && !(dkim_domain = expand_cstring(s)))
  ...
  { errwhen = cUS("dkim_domain"); goto expand_bad; }
...
if (dkim_domain)
  ...
  if (!(dkim_sel = expand_string(dkim->dkim_selector)))
    { errwhen = cUS("dkim_selector"); goto expand_bad; }
  ...
  while ((dkim_signing_selector = string_nextinlist(&dkim_sel, &sel_sep,
    ...
    dkim_canon_expanded = dkim->dkim_canon
      ? expand_string(dkim->dkim_canon) : cUS("relaxed");
    if (!dkim_canon_expanded)	/* expansion error, do not send message. */
      { errwhen = cUS("dkim_canon"); goto expand_bad; }
	...
	{ errwhen = cUS("dkim_timestamps"); goto expand_bad; }
      else
...
if (!dkim_sign_ctx.sig && !dkim->force_bodyhash)
  ...
  if (!sig)
    ...
    sigbuf = string_append(sigbuf, 2, US(sig->signature_header), cUS("\r\n"));
  }
...
for (pdkim_signature * sig = dkim_signatures; sig; sig = sig->next)
  ...
  if (sig->verify_status & PDKIM_VERIFY_POLICY)
    g = string_append(g, 5,
      cUS("policy ("), dkim_verify_status, cUS(" - "), dkim_verify_reason, cUS(")"));
  else switch(sig->verify_status)
    {
    case PDKIM_VERIFY_NONE:    g = string_cat(g, cUS("none")); break;
    case PDKIM_VERIFY_INVALID:
      switch (sig->verify_ext_status)
	{
	case PDKIM_VERIFY_INVALID_PUBKEY_UNAVAILABLE:
          g = string_cat(g, cUS("tmperror (pubkey unavailable)\n\t\t")); break;
        case PDKIM_VERIFY_INVALID_BUFFER_SIZE:
          g = string_cat(g, cUS("permerror (overlong public key record)\n\t\t")); break;
        case PDKIM_VERIFY_INVALID_PUBKEY_DNSRECORD:
        case PDKIM_VERIFY_INVALID_PUBKEY_IMPORT:
          g = string_cat(g, cUS("neutral (public key record import problem)\n\t\t"));
          break;
        case PDKIM_VERIFY_INVALID_SIGNATURE_ERROR:
          g = string_cat(g, cUS("neutral (signature tag missing or invalid)\n\t\t"));
          break;
        case PDKIM_VERIFY_INVALID_DKIM_VERSION:
          g = string_cat(g, cUS("neutral (unsupported DKIM version)\n\t\t"));
          break;
        default:
          g = string_cat(g, cUS("permerror (unspecified problem)\n\t\t")); break;
	}
      break;
    case PDKIM_VERIFY_FAIL:
      switch (sig->verify_ext_status)
	...
	case PDKIM_VERIFY_FAIL_BODY:
          g = string_cat(g,
	    cUS("fail (body hash mismatch; body probably modified in transit)\n\t\t"));
	  break;
        case PDKIM_VERIFY_FAIL_MESSAGE:
          g = string_cat(g,
	    cUS("fail (signature did not verify; headers probably modified in transit)\n\t\t"));
	  break;
        case PDKIM_VERIFY_INVALID_PUBKEY_KEYSIZE:	/* should this really be "polcy"? */
          ...
          break;
        default:
          g = string_cat(g, cUS("fail (unspecified reason)\n\t\t"));
	  break;
	}
      break;
    case PDKIM_VERIFY_PASS:    g = string_cat(g, cUS("pass")); break;
    default:                   g = string_cat(g, cUS("permerror")); break;
    }
  if (sig->domain)   g = string_append(g, 2, cUS(" header.d="), sig->domain);
  if (sig->identity) g = string_append(g, 2, cUS(" header.i="), sig->identity);
  if (sig->selector) g = string_append(g, 2, cUS(" header.s="), sig->selector);
  g = string_append(g, 2, cUS(" header.a="), dkim_sig_to_a_tag(sig));
  }
