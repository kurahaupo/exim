  ...
  *addr_new = next;
  ...
  if (testflag(next, af_pfr))
    ...
    if (next->address[0] == '|')
      ...
      if (rf_get_transport(ob->pipe_transport_name, &ob->pipe_transport,
          next, rblock->name, cUS("pipe_transport")))
      ...
      if (rf_get_transport(ob->reply_transport_name, &ob->reply_transport,
          next, rblock->name, cUS("reply_transport")))
  ...
  DEBUG(D_route)
    ...
    debug_printf("%s router generated %s\n  %serrors_to=%s transport=%s\n",
      ...
      (next->transport == NULL)? cUS("NULL") : next->transport->name);
...
cuschar *save_qualify_domain_recipient = qualify_domain_recipient;
uschar *discarded = cUS("discarded");
...
switch (frc)
  ...
  case FF_FAIL:
    ...
    SORT_MESSAGE:
    if (!addr->message)
      addr->message = yield == FAIL ? cUS("forced rejection") : cUS("forced defer");
    else
      {
      uschar * matched;
  ...
  case FF_FREEZE:
	...
	!= OK) return xrc;
      add_generated(rblock, addr_new, addr, generated, &addr_prop, &ugid, pw);
      if (addr->message == NULL) addr->message = cUS("frozen by filter");
...
if (eblock != NULL)
  ...
  if (filtertype != FILTER_FORWARD || generated == NULL)
    {
    addr->message = cUS("syntax error in redirection data");
    return DECLINE;
    }
  }
...
address as parent. */
  ...
  *addr_new = next;
  ...
  DEBUG(D_route) debug_printf("%s router autogenerated %s\n%s%s%s",
    ...
    (addr_prop.errors_address != NULL)? addr_prop.errors_address : cUS(""),
    (addr_prop.errors_address != NULL)? "\n" : "");
  }
